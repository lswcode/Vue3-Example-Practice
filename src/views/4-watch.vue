...
<script lang='ts'>
import { defineComponent, watch, reactive, toRef } from "vue";
  ...
  setup() {
    ...
    watch(
      ...
      (n, o) => {
        ...
        console.log("旧的值1: ", o, "----", "新的值: ", n);
      }
    );
    watch(
      () => state.age, // 监听对象中的属性，方法1，使用函数格式，return对象属性
      (n, o) => {
        console.log("旧的值2: ", o, "----", "新的值: ", n);
      }
    );
    let name = toRef(state, "name");
    watch(
      name, // 监听对象中的属性，方法2，使用toRef，将指定属性转换为ref格式的响应式数据，然后就可以监听了，不需要加.value
      (n, o) => {
        console.log("旧的值3: ", o, "----", "新的值: ", n);
      }
    );
    watch(
